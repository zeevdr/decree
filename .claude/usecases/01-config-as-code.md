...
1. **Schema in git** — the schema YAML lives in the project repo. On each deploy/startup, the schema is synced to CCS (only if changed). The repo is the source of truth for structure.

2. **Baseline config values in git** — a YAML file per environment (dev, staging, prod) with config values. On deploy, these are applied to CCS. The repo is the source of truth for defaults/baselines.

3. **Runtime overrides via CCS** — operators can change values at runtime via CLI/SDK without redeploying. These runtime changes override the baseline from git.
...
ccs config import <tenant-id> config/values.prod.yaml --description "deploy $(git rev-parse --short HEAD)"
```

### At runtime:
- Operators use `ccs config set` or the SDK to override specific values
...
`ccs schema import --publish` imports the schema YAML and auto-publishes if a new version was created. Single command, no new abstractions.
...
- Current behavior of `ccs config import`
...
- [ ] `--publish` flag on `ccs schema import` (and adminclient `ImportSchema`)
- [ ] `--mode` flag on `ccs config import` (merge/replace/defaults)
...
## Related
- CLI power tools effort (08 Phase 3) — `ccs seed` command wraps both
